...
{
    uint32_t* p_lm;
	p_lm = (uint32_t*)&v_flash;
	for(int ind = 0; ind < sizeof(v_flash)/4 ; ind ++)
	{
	    *p_lm = readFlash(ind);
		   p_lm++;
	}
 ...
 void MC_Save_Setup(void)
{
	      ...
	      p_lm = (uint32_t*)&v_flash;
		 	  writeFlash(sizeof(v_flash)/4, p_lm); // SAVE DATA
...
{
	sise_stup = sizeof(v_flash)/4;

				 writeFlash(sise_stup, (uint32_t*) &v_flash); // SAVE DATA
 }




uint32_t startAddress = 0x08007C00;
/************************************************************
